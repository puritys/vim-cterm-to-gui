...
def cterm2Gui(line):
    ...
    # Get hi xxx
    mats = re.search("hi[\s]+[a-z0-9\_]+[\s]", line, re.I)
    if not mats:
        return ""
    base = line[mats.start():mats.end()]

    # Get cterm
    mats = re.compile("cterm[\s]*=[\s]*([a-z]{1,10})").findall(line, re.I)
    if mats:
        for mat in mats:
            cterm=mat
    ...
    # Get ctermfg
    mats = re.compile("ctermfg[\s]*=[\s]*([0-9]{1,3})").findall(line, re.I)
    if mats:
        for mat in mats:
            ctermfg=mat

    # Get ctermbg
    mats = re.compile("ctermbg[\s]*=[\s]*([0-9]{1,3})").findall(line, re.I)
    if mats:
        for mat in mats:
            ctermbg=mat
    ...
    if base:
        fgRgbColor = xterm2Rgb(ctermfg)
        bgRgbColor = xterm2Rgb(ctermbg);
        result = base + "cterm=" + cterm + " ctermfg="+ctermfg + " ctermbg=" + ctermbg + " guifg=" + fgRgbColor + " guibg=" + bgRgbColor
    return result
